function formatSourceCards(sources) {
  return sources.map((s) => {
    if (['quran', 'quran_translation', 'tafsir'].includes(s.source_type)) {
      ...
      return { type: s.source_type, badge, surahName: s.surah_name_en || s.surah_name_ar, surahNumber: s.surah_number, ayah: s.ayah_number || s.ayah_range, arabic: s.arabic_text, translation: s.translation_text || s.summary, usedFor: 'Islamic evidence', copyCitation: `${s.surah_name_en || s.surah_name_ar || badge} (${s.surah_number || '?'}:${s.ayah_number || s.ayah_range || '?'})` };
    }
    if (['hadith', 'hadith_explanation'].includes(s.source_type)) {
      return { type: s.source_type, badge: 'Hadith', collection: s.collection_name, bookChapter: [s.book_name, s.chapter_name].filter(Boolean).join(' / '), hadithNumber: s.hadith_number || 'Hadith number not available in this source.', grade: s.grade, arabic: s.arabic_text, translation: s.translation_text || s.summary, usedFor: 'Islamic evidence', weakWarning: String(s.grade || '').toLowerCase().includes('weak') ? 'This hadith is graded weak in the approved source. It should not be used as main evidence for a ruling.' : null, copyCitation: `${s.collection_name || 'Hadith'} #${s.hadith_number || 'N/A'}` };
    }
